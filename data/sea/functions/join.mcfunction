...
execute unless entity @a[team=Sea] run execute unless entity @s[nbt={Inventory:[{id:"minecraft:netherite_scrap"},{id:"minecraft:kelp"}]}] run tellraw @s {"text":"YOU DO NOT HAVE THE CORRECT MATERIALS","color":"red"}
execute if entity @s[nbt={Inventory:[{id:"minecraft:netherite_scrap"},{id:"minecraft:kelp"}]}] run tellraw @s[team=Sea] {"text":"YOU ARE ALREADY THE SEA DEITY","color":"red"}
